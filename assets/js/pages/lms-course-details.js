...
;
/* SOURCE: assets/js/lms-course-details.js */
/* SCREENINGS4U LMS — COURSE DETAILS / PURCHASE PREVIEW */
(function(){
"use strict";
...
async function init(){
  ...
  var q=db.from("lms_courses").select("id,slug,title,short_description,description,status,navigation_mode,certificate_enabled");
  q=id?q.eq("id",id):q.eq("slug",slug);
  var cr=await q.maybeSingle();
  if(cr.error) throw cr.error;
  if(!cr.data) throw new Error("This course is unavailable.");
  course=cr.data;

  var er=await db.from("lms_enrollments")
    .select("id,status,progress_percent,expires_at")
    .eq("user_id",user.id)
  ...
  // SECURITY/PREVIEW RULE:
  // Do not query sections or lessons for a learner who has not purchased/been assigned the course.
  // This keeps the view page a true sales preview and prevents lesson records/content from appearing
  // in browser developer tools before enrollment.
  if(enrollment){
    var sr=await db.from("lms_sections")
    ...
    var sectionIds=sections.map(x=>x.id), lessons=[];
    if(sectionIds.length){
      var lr=await db.from("lms_lessons")
        .select("id,section_id,title,estimated_minutes,sort_order,status")
...
function render(){
  ...
  set("[data-course-category]","Training");

  var cleanShort=cleanCourseText(course.short_description||course.description||"");
  var cleanFull=cleanCourseText(course.description||course.short_description||"");
  set("[data-course-description]",excerpt(cleanShort||cleanFull,220));

  var all=enrollment?sections.flatMap(s=>s.lessons):[];
  var mins=all.reduce((n,l)=>n+(Number(l.estimated_minutes)||0),0);
  var meta=document.querySelectorAll(".course-details-meta-item");
  if(meta[0]) setLast(meta[0],"Self-paced");
  if(meta[1]) setLast(meta[1],enrollment?(all.length+" lesson"+(all.length===1?"":"s")):"Course access after purchase");
  ...
  if(about[0]) about[0].textContent=excerpt(cleanFull||cleanShort,380)||"Review the course overview and learning outcomes before purchasing full access.";
  if(about[1]) about[1].textContent=enrollment?"Your progress is saved to your Screenings4u learning record.":"Lesson content, quizzes, assessments, and training materials unlock after purchase or assignment.";

  var intro=document.querySelector(".course-details-section:nth-of-type(3) .course-details-section-text");
  var curriculum=document.getElementById("courseCurriculum");
  if(enrollment){
    if(intro) intro.textContent=`Your enrollment includes ${all.length} lesson${all.length===1?"":"s"} organized into ${sections.length} module${sections.length===1?"":"s"}.`;
    if(curriculum) curriculum.innerHTML=sections.length?sections.map(moduleHtml).join(""):'<div class="course-preview-locked"><strong>Course content is being prepared.</strong><span>No published lesson outline is available yet.</span></div>';
    bindModules();
  } else {
    if(intro) intro.textContent="The full course curriculum is protected. Purchase or assignment is required before lesson titles, modules, quizzes, assessments, and course materials are loaded.";
    if(curriculum) curriculum.innerHTML=`<div class="course-preview-locked"><div class="course-preview-lock-icon">🔒</div><div><strong>Full curriculum unlocks after purchase</strong><span>This preview intentionally does not load the complete course or lesson records.</span></div></div>`;
  }

  var vals=document.querySelectorAll(".course-details-info-value");
  if(vals[0]) vals[0].textContent=enrollment&&mins?formatMinutes(mins):"Self-paced";
  if(vals[1]) vals[1].textContent=enrollment?(all.length+" lessons"):"Unlocked after purchase";
  ...
  if(enrollment){
    ...
    if(action){
      action.href=`lms-course-player.html?course=${encodeURIComponent(course.id)}&enrollment=${encodeURIComponent(enrollment.id)}`;
      action.textContent=enrollment.status==="completed"?"Review Course":"Continue Learning";
    }
    if(note) note.textContent="Your course access is connected to this enrollment.";
  } else {
    if(label) label.textContent="Review & Purchase";
    if(title) title.textContent="Get Full Course Access";
    if(text) text.textContent="Purchase this course to unlock all lessons, quizzes, assessments, materials, and certificate requirements.";
    if(rows[0]) rows[0].textContent="Purchase required";
    if(rows[1]) rows[1].textContent="Preview only";
    if(rows[2]) rows[2].textContent="Self-paced";
    if(action){
      action.href=`lms-checkout.html?course=${encodeURIComponent(course.id)}`;
      action.textContent="Purchase Full Access";
    }
    if(note) note.textContent="Secure checkout completes your purchase. After payment, the course appears in My Learning.";
  }
}
...
function cleanCourseText(value){var text=String(value==null?"":value);if(!text)return "";for(var i=0;i<2;i++){text=text.replace(/<br\s*\/?>/gi," ").replace(/<\/p\s*>/gi," ").replace(/<\/li\s*>/gi," ").replace(/<li[^>]*>/gi," ");var decoder=document.createElement("textarea");decoder.innerHTML=text;text=decoder.value;var holder=document.createElement("div");holder.innerHTML=text;text=holder.textContent||holder.innerText||"";}return text.replace(/\u00a0/g," ").replace(/\s+/g," ").trim();}
function excerpt(value,maxLength){var text=cleanCourseText(value);if(!text||text.length<=maxLength)return text;var slice=text.slice(0,maxLength+1),space=slice.lastIndexOf(" ");return slice.slice(0,space>0?space:maxLength).trim()+"…";}
