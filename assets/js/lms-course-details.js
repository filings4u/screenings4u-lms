...
function render(){
 ...
 var rawShort=course.short_description||course.description||"";
 var rawFull=course.description||course.short_description||"";
 var cleanShort=cleanCourseText(rawShort);
 var cleanFull=cleanCourseText(rawFull);
 set("[data-course-description]",excerpt(cleanShort||cleanFull,240));
 ...
 if(about[0]) about[0].textContent=excerpt(cleanFull||cleanShort,520)||"Complete this course at your own pace.";
 ...
 if(enrollment){
   ...
   if(rows[2])rows[2].textContent=mins?formatMinutes(mins):"Self-paced";
   if(action){action.href=`lms-checkout.html?course=${encodeURIComponent(course.id)}`;action.textContent="Purchase Full Access";}
...
function cleanCourseText(value){
 ...
 if(!text)return "";
 for(var i=0;i<2;i++){
   text=text.replace(/<br\s*\/?>/gi," ").replace(/<\/p\s*>/gi," ").replace(/<\/li\s*>/gi," ").replace(/<li[^>]*>/gi," ");
   var decoder=document.createElement("textarea");decoder.innerHTML=text;text=decoder.value;
   var holder=document.createElement("div");holder.innerHTML=text;text=holder.textContent||holder.innerText||"";
 }
 return text.replace(/\u00a0/g," ").replace(/\s+/g," ").trim();
}
function excerpt(value,maxLength){
 var text=cleanCourseText(value);if(!text||text.length<=maxLength)return text;
 var slice=text.slice(0,maxLength+1),space=slice.lastIndexOf(" ");
 return slice.slice(0,space>0?space:maxLength).trim()+"…";
}
